# Frontend Mentor - Single Price Component

This is a solution to the [Single price grid component](https://www.frontendmentor.io/challenges/single-price-grid-component-5ce41129d0ff452fec5abbbc-). Frontend Mentor challenges help you improve your coding skills by building realistic projects. 

## Table of contents

- [Overview](#overview)
  - [Screenshot](#screenshot)
  - [Links](#links)
- [My process](#my-process)
  - [Built with](#built-with)
  - [What I learned](#what-i-learned)
  - [Continued development](#continued-development)
  - [Useful resources](#useful-resources)
- [Author](#author)

## Overview

Single Pirce component created with HTML and Tailwind CSS
### Screenshot

Mobile

![image](https://user-images.githubusercontent.com/90436675/164913806-575a0d7b-3b36-4932-940e-998d0230c56e.png)


Desktop

![image](https://user-images.githubusercontent.com/90436675/164913789-ae254a90-c8d0-450b-a348-8fda13716558.png)


### Links

- Solution URL: [website](https://single-price-me.netlify.app/)
- Live Site URL: [GitHub](https://github.com/Marvin-Erazo/single-price-grid-component)

## My process

### Built with

- Semantic HTML5 markup
- Tailwind CSS (via CDN)

### What I learned

Tailwind classes


### Continued development

Tailwind Grids and flex components, also animations and tranforms

### Useful resources

- [TW responsive](https://tailwindcss.com/docs/responsive-design) - This helped me for set the width of screens
- [TW classes](https://tailwind.build/classes) - All TW classes

## Author

- Website - [Marvin Erazo](https://marvin-erazo.github.io/)
- Frontend Mentor - [@Marvin-Erazo](https://www.frontendmentor.io/profile/Marvin-Erazo)
- GitHub - [Marvin-Erazo](https://github.com/Marvin-Erazo)
- Linkedn - [Marvin Erazo](https://www.linkedin.com/in/marvin-erazo-b971a7221/)
